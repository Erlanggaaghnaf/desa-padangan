document.addEventListener("DOMContentLoaded", function() {
    ...
    // Fungsi untuk mengatur berita ditambahkan penanda 'isLoadingAwal'
    function updateTampilan(isLoadingAwal = false) {
        ...
        // HANYA scroll ke atas daftar berita jika ini BUKAN loading awal (artinya saat tombol diklik)
        if (!isLoadingAwal) {
            // Memberikan sedikit jeda (setTimeout) agar transisi scroll lebih stabil
            setTimeout(() => {
                const headerOffset = 100; // Jarak aman dari navbar atas
                const elementPosition = document.getElementById('berita-container').getBoundingClientRect().top;
    ...
    // Jalankan fungsi saat halaman pertama kali dimuat dan kirim sinyal "true" (Ini Loading Awal)
    updateTampilan(true);
});